...
of a software are tested. This is used to validate that each unit of the
...
test fixture:
=============
A test fixture is used as a baseline for running tests to ensure that
there is a fixed environment in which tests are run so that results are repeatable.
Examples :
...
test suite:
===========
Test suite is a collection of testcases that are used to test a software program
to show that it has some specified set of behaviours by executing
the aggregated tests together.
...
if __name__ == '__main__':
    unittest.main()
'''
This is the basic test code using unittest framework, which is having a single test. 
...
“-v” option is added in the command line while running the tests to obtain more 
...
assertEqual() – This statement is used to check if the result obtained is equal to 
the expected result.
assertTrue() / assertFalse() – This statement is used to verify if a given statement 
is true or false.
...
string through the argument passed in the function and returns the result as list. 

The assertEqual() statement returns true in this case if the result matches the given output.
unittest.main() provides a command-line interface to the test script.On running the above script from the command line
'''